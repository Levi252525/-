...
function resetGame() {
  ...
  resetPlayerToSpawn();
  resetBots();
  hudStatus.textContent = "No score / no checkpoints - 20 platform-follow bots";
}
...
function updateSingleBot(bot) {
  ...
  const targetX = target ? target.aimX : playerCenterX;
  const targetAbove = Boolean(target && target.platform.y < bot.y - 8);
  ...
  const playerAboveBot = player.y < bot.y - 40;
  const horizontallyReady = Math.abs(targetX - botCenterX) < 26;
  const onBottomFloor = bot.y + bot.h >= WORLD.height - 34;

  if (bot.onGround && bot.jumpCooldown <= 0) {
    const shouldGroundJump = (targetAbove && horizontallyReady) || (onBottomFloor && playerAboveBot);
    if (shouldGroundJump) {
      bot.vy = -(bot.jumpStrength + (playerAboveBot ? 0.4 : 0));
      bot.onGround = false;
      bot.jumpsUsed = 1;
      bot.jumpCooldown = 12;
    }
  } else if (!bot.onGround && bot.jumpsUsed < bot.maxJumps && bot.jumpCooldown <= 0) {
    // Use second jump near the arc apex to chain onto higher platforms.
    const nearApex = bot.vy > -0.7;
    const alignedForSecondJump = Math.abs(targetX - botCenterX) < 88;
    if (targetAbove && nearApex && alignedForSecondJump) {
      bot.vy = -(bot.jumpStrength - 0.8);
      bot.jumpsUsed += 1;
      bot.jumpCooldown = 14;
    }
  }

  if (bot.onGround && Math.abs(playerCenterX - botCenterX) < 10 && playerAboveBot && bot.jumpCooldown <= 0) {
